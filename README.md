...
- Транскрипция готовых аудиофайлов (кнопка 📁): mp3, wav, flac, ogg, aiff. Результат сохраняется в `.txt` рядом с файлом и копируется в буфер.
- Разделение стерео-каналов (тумблер «Split»): левый канал — `Я`, правый — `Собеседник`, диалог по очереди. Удобно для записей звонков.
...
[**Скачать последний DMG**](https://github.com/OlegArapov/MacDictator/releases/latest)
