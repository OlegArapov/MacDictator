...
# py2app не упаковывает скрипты Tcl/Tk (init.tcl, tk.tcl) — только dylib'ы.
# В .app они лежат в Resources/lib; направляем Tcl/Tk на них до загрузки tkinter,
# иначе на чистом Mac без Homebrew tcl-tk зашитый путь к init.tcl не существует
# и приложение падает на import customtkinter.
if getattr(sys, "frozen", False):
    _res = os.environ.get("RESOURCEPATH") or os.path.join(
        os.path.dirname(sys.executable), os.pardir, "Resources")
    _tcl_lib = os.path.join(_res, "lib", "tcl9.0")
    _tk_lib = os.path.join(_res, "lib", "tk9.0")
...
UPDATE_CHECK_URL = "https://api.github.com/repos/OlegArapov/MacDictator/releases/latest"


def _parse_version(s):
    """Best-effort version tuple from 'v1.2.3' or '1.2.3'. Returns (0,0,0) on garbage."""
    if not s:
        return (0, 0, 0)
...
def _fetch_latest_release():
    """Hit GitHub releases API. Returns (tag, html_url) or None on any failure."""
    try:
        # Frozen .app has no system CA paths for urllib; use certifi (ships with openai/httpx).
        import ssl
        try:
            import certifi
        ...
        req = Request(UPDATE_CHECK_URL, headers={
            "User-Agent": f"MacDictator/{APP_VERSION}",
        ...
        # Passive feature: any failure (network, TLS, truncated body) stays silent
        return None


def _atomic_write_json(path, data, *, ensure_ascii=True, indent=2):
    """Write JSON atomically: tmp file + fsync + rename. Survives crashes mid-write."""
    tmp = path + ".tmp"
...
def _save_keys(keys):
    _atomic_write_json(KEYS_FILE, keys)
...
DEFAULTS = {"engine": "MLX", "model": "large-v3", "translate": "Off", "translate_model": "DeepSeek", "cleanup": "Off", "cleanup_model": "DeepSeek", "send": "Off", "separate_channels": "Off", "pill_hidden": False, "last_seen_version": "0.0.0"}
...
def _mlx_model_downloaded(repo):
    ...
    snapshot_download(local_files_only=True) returns success even when files are
    missing or empty if the snapshot dir exists, so we also verify size.
    """
    if not mlx_whisper:
        return False
    try:
        from huggingface_hub import snapshot_download
        snap_path = snapshot_download(repo, local_files_only=True)
        # Sanity-check: total snapshot size should be at least ~90% of expected.
        total = 0
        for root, _, files in os.walk(snap_path, followlinks=True):
            for f in files:
                try:
...
def _save_prompts(prompts):
    _atomic_write_json(PROMPTS_FILE, prompts, ensure_ascii=False)
...
class RecordingBubble(ctk.CTkToplevel):
    ...
    def _tick(self):
        ...
        # Тёмная капсула, мягкий серый контур — без красной рамки
        r = h // 2
        ...
        # Спокойная красная точка записи — единственный насыщенный красный
        c.create_oval(9, mid - 4, 17, mid + 4, fill="#DC2626", outline="")

        # Эквалайзер: 7 широких баров, колокол к центру, лёгкое шевеление по уровню
        weights = (0.5, 0.72, 0.9, 1.0, 0.9, 0.72, 0.5)
        x0, region = 26, 62
        slot = region / len(weights)
        bw = slot * 0.62
        max_amp = mid - 3
        for i, wgt in enumerate(weights):
            life = 1.0 + 0.18 * float(np.sin(elapsed * 7 + i * 0.9)) * self._smooth
            amp = max(1, int(self._smooth * wgt * life * max_amp))
            cx = x0 + i * slot + slot / 2
            color = "#F87171" if amp > 1 else "#41414F"
            c.create_rectangle(cx - bw / 2, mid - amp, cx + bw / 2, mid + amp,
                               fill=color, outline="")
...
class DictatorApp(ctk.CTk):
    ...
    @staticmethod
    def _clip_edit(event, action):
        """Cmd+C/V/X/A для Entry/Text на macOS Tk 9 (нативные биндинги мертвы)."""
        w = event.widget
        is_text = w.winfo_class() == "Text"
        try:
            if action == "paste":
                try:
    ...
    def _install_clipboard_shortcuts(self):
        for cls in ("Entry", "Text"):
            self.bind_class(cls, "<Command-v>", lambda e: self._clip_edit(e, "paste"))
            self.bind_class(cls, "<Command-c>", lambda e: self._clip_edit(e, "copy"))
            self.bind_class(cls, "<Command-x>", lambda e: self._clip_edit(e, "cut"))
            self.bind_class(cls, "<Command-a>", lambda e: self._clip_edit(e, "all"))

    def __init__(self):
        super().__init__()

        # macOS Tk 9: у класса Entry/Text нет Cmd+C/V/X/A — стандартный <<Paste>>
        # привязан к Mod1, а Command в Tk 9 на aqua больше не Mod1, поэтому вставка
        # в поля ввода (в т.ч. API-ключи и промпты) не работает. Вешаем правки на
        # классы Entry и Text один раз — покрывает все поля приложения.
        ...
        # Require the pill fully on-screen (saved pos may be from an external
        # monitor or land behind the Dock, leaving it effectively invisible)
        if ox < 0 or ox + pw > sw or oy < 0 or oy + ph > sh - 60:
        ...
        # Click on the Dock icon toggles the pill (macOS "reopen" event)
        try:
            self.createcommand('::tk::mac::ReopenApplication', self._toggle_visibility)
        except Exception as e:
            logging.warning("Dock reopen handler: %s", e)
        # Update check: fire once a few seconds after launch (don't slow startup).
    ...
    def _save_settings(self):
        try:
            # Merge into the in-memory dict so keys not owned by this form
            # (pill_hidden, last_seen_version) survive, and so other write
            # paths (_persist_settings) never see a stale snapshot.
            ...
            _atomic_write_json(SETTINGS_FILE, self._settings, ensure_ascii=False)
            self._update_mode_label()
    ...
    def _persist_settings(self):
        """Write self._settings as-is (for keys changed outside the settings form)."""
        try:
            _atomic_write_json(SETTINGS_FILE, self._settings, ensure_ascii=False)
        except Exception as e:
            logging.warning("Failed to persist settings: %s", e)
    ...
    def _save_history(self):
        try:
            _atomic_write_json(HISTORY_FILE, self.history[-10:], ensure_ascii=False)
        except Exception as e:
    ...
    def _build_ui(self):
        ...
        self._hide_btn = ctk.CTkButton(
            right, text="✕", width=18, height=14,
            font=("SF Pro Text", 10), corner_radius=3,
            fg_color="transparent", text_color=C["text3"],
            hover_color=C["hover"],
            command=self._toggle_visibility)
        self._hide_btn.pack(anchor="e", pady=(0, 1))

        # ⚙ settings + 📁 open-file share one row so the pill stays 3 rows tall
        actions_row = ctk.CTkFrame(right, fg_color="transparent")
    ...
    def _open_settings_popup(self):
        ...
        if self._update_tag:
            ver_lbl = ctk.CTkLabel(
                footer, text=f"v{APP_VERSION} → {self._update_tag} download",
                font=("SF Mono", 9, "underline"), text_color=C["green"], cursor="hand2")
            ver_lbl.bind("<Button-1>", lambda e: self._open_update_url())
        else:
            ver_lbl = ctk.CTkLabel(
    ...
    def _show_frameless(self):
        if self._settings.get("pill_hidden", False):
            # Stay withdrawn; user can summon with Ctrl+Shift+H.
    ...
    def _toggle_visibility(self):
        ...
        if visible:
            self.withdraw()
            self._settings["pill_hidden"] = True
        else:
            self.overrideredirect(True)
            self.deiconify()
            self.attributes('-topmost', True)
            ...
            self._settings["pill_hidden"] = False
        self._persist_settings()

    def _start_update_check(self):
        """Background GitHub Releases check. Silent on any failure."""
        def _check():
            ...
            last_seen = _parse_version(self._settings.get("last_seen_version", "0.0.0"))
            if latest <= current or latest <= last_seen:
                return
            self.after(0, lambda: self._on_update_available(tag, url))
    ...
    def _on_update_available(self, tag, url):
        ...
        if self.app_state == self.STATE_IDLE:
            self.update_status(f"Update {tag} →", "green")
    ...
    def _open_update_url(self):
        ...
        try:
            ...
            logging.warning("Failed to open update URL: %s", e)
        # User saw it; don't nag again this version.
        self._settings["last_seen_version"] = self._update_tag or APP_VERSION
        self._persist_settings()
    ...
    def start_keyboard_listener(self):
        ...
        def on_press(key):
            ...
            shift = kb.Key.shift_l in pressed or kb.Key.shift_r in pressed
            if ctrl and shift and key == kb.Key.space:
        ...
        # Watchdog: restart listener on crash, surface a UI warning after repeated failures.
        def _run_listener():
            failures = []
            warned = False
            while True:
                try:
                    with kb.Listener(on_press=on_press, on_release=on_release) as listener:
                        listener.join()
                    # Clean exit (rare): bail out.
                    logging.warning("Keyboard listener exited cleanly")
                    break
                except Exception as e:
                    logging.warning("Keyboard listener crashed: %s", e)
                    now = time.time()
                    failures = [t for t in failures if now - t < 30] + [now]
                    if len(failures) >= 3 and not warned:
                        warned = True
                        self.after(0, lambda: self.update_status(
                            "Hotkey broken — use button", "red"))
                        return  # stop retrying; click-only mode
                    time.sleep(1)
    ...
    def _init_stream(self):
        ...
        self._mic_timer = None
        # Volume polled by main thread; audio callback only writes, never touches Tk.
        self._latest_rms = 0.0
        self._vu_poll_job = None
        self._open_mic()
        self._start_vu_poll()
    ...
    def _audio_always_callback(self, indata, frames, time_info, status):
        # Runs on PortAudio realtime thread — NO Tk access, NO logging from here.
        if status:
            self._stream_error = True
        if self._recording:
            self.audio_data.append(indata.copy())
        # Single float assignment; atomic under CPython GIL.
        self._latest_rms = float(np.sqrt(np.mean(indata ** 2)))

    def _start_vu_poll(self):
        if self._vu_poll_job is not None:
            return
        self._vu_poll_job = self.after(50, self._vu_poll_tick)
    ...
    def _vu_poll_tick(self):
        ...
        try:
            ...
            if self._recording:
                vol = min(1.0, rms * 5.0)
                self.vu.set_volume(vol)
                self._rec_bubble.set_volume(vol)
            elif self._mic_live:
                self.vu.set_volume(min(1.0, rms * 5.0))
        except Exception as e:
            logging.warning("VU poll error: %s", e)
        self._vu_poll_job = self.after(50, self._vu_poll_tick)
    ...
    def _whisper_transcribe_array(self, audio, lang, want_segments=False):
        """Транскрибировать 1-D float32 массив (16 кГц).

        want_segments=False → склеенный текст; True → список {start, text}
        с абсолютным временем в переданном аудио.
        """
                ...
                try:
                    with _mlx_lock:
                        r = mlx_whisper.transcribe(
                            _a, path_or_hf_repo=repo, language=lang, **_whisper_kwargs)
                    _result_box[0] = r
                except Exception as e:
                    _result_box[1] = e
    ...
    def process_audio(self):
        ...
        try:
                ...
                try:
                    with open(filename, "rb") as f:
                        resp = openai_client.audio.transcriptions.create(
                            model="whisper-1", file=f, timeout=60.0)
            ...
            if not self.cancelled and send_mode in ("Enter", "Paste"):
                # Marshal CGEventPost calls to main thread; some event types are main-thread-only.
                paste_done = threading.Event()
                def _press_enter():
                    try:
                        pyautogui.press('return')
                    except Exception as e:
                        logging.warning("Failed to press return: %s", e)
                    finally:
                        paste_done.set()
                def _do_paste():
                    try:
                        ...
                        return
                    if send_mode == "Enter":
                        # Let the paste land before Enter — without blocking the UI loop
                        self.after(300, _press_enter)
                    else:
                        paste_done.set()
                self.after(0, _do_paste)
                paste_done.wait(timeout=5.0)
            if self.cancelled:
                return
    ...
    def _pick_file(self):
        if self._processing_lock.locked() or self.app_state in (self.STATE_RECORDING, self.STATE_PROCESSING):
            self.update_status("Занято, дождись окончания", "orange")
            return
        path = filedialog.askopenfilename(
            title="Выбери аудиофайл",
            filetypes=[("Аудио", "*.mp3 *.wav *.flac *.ogg *.aiff *.aif *.m4a"),
                       ("Все файлы", "*.*")])
        if not path:
            return
        if self._processing_lock.locked() or self.app_state in (self.STATE_RECORDING, self.STATE_PROCESSING):
        ...
        self.cancelled = False
        self.app_state = self.STATE_PROCESSING
        self.update_status("Читаю файл...", "orange")
    ...
    def process_file(self, path):
        ...
        try:
            ...
            traceback.print_exc()
            if not self.cancelled:
                self.after(0, lambda: self._on_error("Формат не поддерживается, сконвертируй в mp3/wav"))
        except Exception as e:
            import traceback
            traceback.print_exc()
            msg = str(e)[:80]
